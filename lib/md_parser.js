...
var ParseEngine=require('./ParseEngine');
var defaultJSON = {
    title: 'NodePPt In MarkDown',
    url: '',
    speaker: '',
    content: '',
    theme: 'moon',
...
var parser = function (string, callback, argvObj, queryObj, commonData) {
    ...
    var slidesSetting = string.match(ParseEngine.slideToken) || [];

    var contents = string.split(/\[slide.*\]|\[幻片.*\]/i);
     //debug.log('内容原文：' + contents.length, '设置：' + slidesSetting);
    ...
    contents.forEach(function (v, i) {
        ...
        cfg = cfg.match(ParseEngine.oneSlideToken);
        var attrs = '';
        if (cfg) {
            attrs = cfg[1];
        }
        var note = ParseEngine.noteToken.exec(v);
        if (note) {
            v = v.replace(ParseEngine.noteToken, '');
            note = marked(note[1]);
        }
        var subslide = ParseEngine.subslideToken.exec(v);
        var extData = [],
            extType = '',
            extAttr = '';
        if (subslide) {
            ...
            if (subslide.length > 1) {
                v = v.replace(ParseEngine.subslideToken, EOL + SUB_SILDE_PLACEHOLDER + EOL);
                subslide.forEach(function (v, i) {
                    extData.push(marked(v));
                });
            } else {
                v = v.replace(ParseEngine.subslideToken, subslide[0]);
            }
        }
        var magicSlide = ParseEngine.magicToken.exec(v);
        if (magicSlide) {
            extType = 'magic';
            ...
            if (magicSlide.length > 1) {
                v = v.replace(ParseEngine.magicToken, EOL + SUB_SILDE_PLACEHOLDER + EOL);
                magicSlide.forEach(function (v, i) {
                    extData.push(['<div class="magic-wrapper">', contentParser(marked(v)), '</div>'].join(EOL));
                });
            } else {
                v = v.replace(ParseEngine.magicToken, magicSlide[0]);
            }
        }
        v = v.trim();
        var tContent = '';
...
function parseCover(str) {
    ...
    str.forEach(function (val) {
        if (val = ParseEngine.coverToken.exec(val)) {
            obj[val[1]] = val[2];
        }
    });
    return obj;
    ...
    attrs.forEach(function (attr) {
        if (!!~attr.indexOf('=')) {
            ...
            ids.forEach(function (v) {
                if (v !== '') {
                    ParseEngine.idToken(v).test(idinput) && idArr.push(v);
                    ParseEngine.classToken(v).test(idinput) && classArr.push(v);
                }
            });
        }
    });
